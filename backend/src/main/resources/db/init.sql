...
CREATE TABLE IF NOT EXISTS lineage_relation (
    lineage_id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '血缘ID',
    ...
    relation_type VARCHAR(20) NOT NULL COMMENT '关系类型：ETL/COPY/REPORT/API/MANUAL',
    level INT COMMENT '血缘层级',
    path VARCHAR(500) COMMENT '路径描述',
    confidence INT COMMENT '置信度',
    transform_desc VARCHAR(500) COMMENT '转换逻辑描述',
    source_type VARCHAR(10) DEFAULT 'MANUAL' COMMENT '来源：AUTO/MANUAL',
    status VARCHAR(20) DEFAULT 'ACTIVE' COMMENT '状态',
    source_script VARCHAR(5000) COMMENT '来源脚本',
    creator_id BIGINT,
...
CREATE TABLE IF NOT EXISTS impact_analysis (
    ...
    source_asset_id BIGINT NOT NULL COMMENT '变更资产ID',
    affected_asset_id BIGINT NOT NULL COMMENT '受影响资产ID',
    impact_level VARCHAR(20) NOT NULL COMMENT '影响程度：DIRECT/INDIRECT',
    impact_path TEXT NOT NULL COMMENT '影响路径(JSON数组)',
    path_depth INT NOT NULL COMMENT '路径深度',
    creator_id BIGINT,
    created_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    deleted INT DEFAULT 0,
    updater_id BIGINT,
    ...
    INDEX idx_affected (affected_asset_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='影响分析结果';
...
-- 21. 生命周期策略表
CREATE TABLE IF NOT EXISTS lc_policy (
    policy_id BIGINT PRIMARY KEY AUTO_INCREMENT,
    policy_name VARCHAR(100) NOT NULL COMMENT '策略名称',
    grading_level VARCHAR(20) NOT NULL COMMENT '分级：L1/L2/L3/L4',
    retention_period INT NOT NULL COMMENT '保留期限(天)',
    retention_unit VARCHAR(10) DEFAULT 'DAY' COMMENT '时间单位',
    retention_base VARCHAR(20) DEFAULT 'CREATE_TIME' COMMENT '计算基准',
    pre_expire_days INT DEFAULT 30 COMMENT '提前预警天数',
    expire_action VARCHAR(20) DEFAULT 'ARCHIVE' COMMENT '到期动作：ARCHIVE/DELETE',
    ...
    updated_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted INT DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='生命周期策略';
...
CREATE TABLE IF NOT EXISTS lc_status (
    ...
    lifecycle_stage VARCHAR(20) NOT NULL DEFAULT 'ACTIVE' COMMENT '阶段：ACTIVE/EXPIRING/EXPIRED/ARCHIVED/DELETED',
    expire_date DATE NOT NULL COMMENT '到期日期',
    pre_expire_notified TINYINT(1) DEFAULT 0 COMMENT '是否已预警',
    dispose_status VARCHAR(20) DEFAULT 'PENDING' COMMENT '处置状态',
    dispose_retry_count INT DEFAULT 0 COMMENT '重试次数',
    disposed_time DATETIME COMMENT '处置时间',
    creator_id BIGINT,
    created_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    updater_id BIGINT,
    updated_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='生命周期状态';
...
CREATE TABLE IF NOT EXISTS data_standard (
    ...
    standard_type VARCHAR(20) NOT NULL COMMENT '类型：NAMING/TYPE/CODING/VALUE_DOMAIN',
    standard_content TEXT NOT NULL COMMENT '标准内容(JSON)',
    description VARCHAR(500),
    status VARCHAR(10) DEFAULT 'ENABLED',
    creator_id BIGINT,
    created_time DATETIME DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS compliance_clause (
    clause_id BIGINT PRIMARY KEY AUTO_INCREMENT,
    clause_code VARCHAR(50) NOT NULL,
    regulation_source VARCHAR(100) NOT NULL,
    clause_content VARCHAR(1000) NOT NULL,
    eval_method VARCHAR(20) NOT NULL COMMENT 'AUTO/SEMI_AUTO/MANUAL',
    eval_expression VARCHAR(2000),
    compliance_status VARCHAR(20) DEFAULT 'PENDING',
    eval_evidence VARCHAR(500),
    ...
    updated_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted INT DEFAULT 0,
    UNIQUE KEY idx_code (clause_code)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='合规条款';
...
CREATE TABLE IF NOT EXISTS governance_kpi (
    ...
    updater_id BIGINT,
    updated_time DATETIME,
    UNIQUE KEY idx_code_date (kpi_code, record_date)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='治理KPI';

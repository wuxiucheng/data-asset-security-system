<template>
  <div class="classification-standard-container">
    <!-- 搜索表单 -->
...
import { classificationStandardApi } from '@/api'
...
// 获取分类标准列表
const getClassificationStandardList = async () => {
  loading.value = true
  try {
    // 过滤空值参数
    const params: any = {
    ...
    const res = await classificationStandardApi.getList(params)
    tableData.value = res.data.list
    pagination.total = res.data.total
  } catch (error) {
    ElMessage.error('获取分类标准列表失败')
  } finally {
    loading.value = false
  }
}
...
const handlePublish = async (row: any) => {
  try {
    await ElMessageBox.confirm('确定要发布该分类标准吗？', '提示', {
      type: 'warning'
    })
    await classificationStandardApi.publish(row.standardId)
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm('确定要删除该分类标准吗？', '提示', {
      type: 'warning'
    })
    await classificationStandardApi.delete(row.standardId)
...
const handleSubmit = async () => {
  try {
    ...
    if (isEdit.value) {
      await classificationStandardApi.update(formData)
      ElMessage.success('更新成功')
    } else {
      await classificationStandardApi.create(formData)
...
<style scoped>
.classification-standard-container {
  padding: 20px;
}

...
if [ "$BACKEND_TYPE" = "mock" ]; then
    ...
    echo "用法: ./start-all.sh [mock|real] [build]"
    echo "  mock - 启动Mock后端 + 前端"
    echo "  real - 启动Spring Boot后端 + 前端（默认）"
    echo "  build - 重新编译后端项目"
    echo ""
    echo "示例："
    echo "  ./start-all.sh              # 启动Spring Boot后端（不重新编译）"
    echo "  ./start-all.sh real build   # 启动Spring Boot后端（重新编译）"
    echo "  ./start-all.sh mock         # 启动Mock后端"
    exit 1
fi

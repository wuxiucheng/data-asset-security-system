...
# 检查MySQL
echo ">>> 检查MySQL数据库..."
if ! /usr/local/mysql/bin/mysqladmin ping -uroot -p1q2w3e4r --silent 2>/dev/null; then
...
# 检查数据库是否存在
if ! /usr/local/mysql/bin/mysql -uroot -p1q2w3e4r -e "USE data_asset_security" 2>/dev/null; then
    echo "❌ 数据库 data_asset_security 不存在"
    echo ">>> 创建数据库..."
    /usr/local/mysql/bin/mysql -uroot -p1q2w3e4r -e "CREATE DATABASE data_asset_security CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;"
    echo "✅ 数据库创建成功"
fi
echo ""
...
# 如果JAR文件不存在，或者指定了build参数，则编译
if [ ! -f "$JAR_FILE" ] || [ "$NEED_BUILD" = true ]; then
    echo ">>> 编译Spring Boot项目..."
    cd "$BACKEND_DIR"
    /usr/local/apache-maven-3.8.7/bin/mvn clean package -DskipTests

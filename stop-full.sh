...
if [ -z "$RUNNING_PROCESSES" ]; then
    ...
    if [ -z "$REMAINING_PROCESSES" ]; then
        print_success "所有后端进程已成功停止"
    else
        print_error "仍有后端进程在运行，请手动检查"
        echo ""
...
RUNNING_FRONTEND=$(ps aux | grep "vite" | grep node | grep -v grep)
...
if [ -z "$RUNNING_FRONTEND" ]; then
    ...
    echo "找到以下运行中的前端进程："
    echo ""
    echo "$RUNNING_FRONTEND" | while read -r line; do
        PID=$(echo "$line" | awk '{print $2}')
        echo "  PID: $PID"
        echo "  命令: $line"
        echo ""
    ...
    # 获取所有进程PID
    PIDS=$(ps aux | grep "vite" | grep node | grep -v grep | awk '{print $2}')
    ...
    REMAINING_PROCESSES=$(ps aux | grep "vite" | grep node | grep -v grep)

    if [ -z "$REMAINING_PROCESSES" ]; then
        print_success "所有前端进程已成功停止"
    else
        print_error "仍有前端进程在运行，请手动检查"
        echo ""
        echo "$REMAINING_PROCESSES"
    fi
fi
...
echo "   - 重新启动: $PROJECT_ROOT/start-full.sh"
echo ""
